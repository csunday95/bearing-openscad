module outer_race_profile(height, depth, lip_thickness, lip_depth) {
  points = [
    [0, -height / 2],
    [depth, -height / 2],
    [depth, height / 2],
...
module outer_race(height, id, od, lip_thickness, lip_depth, notches=0, notch_depth=0) {
  thickness = (od - id) / 2;
  difference() {
    rotate_extrude() {
      translate([id / 2, 0, 0])
        outer_race_profile(height, thickness, lip_thickness, lip_depth);
    }
